...
// cache Value*'s names
static unordered_map<const llvm::Value*, string> value_names;
static unsigned value_id_counter; // for %0, %1, etc..

static vector<unique_ptr<IntType>> int_types;
static unsigned type_id_counter; // for unamed types

// cache complex types
static unordered_map<const llvm::Type*, unique_ptr<Type>> type_cache;
static unordered_map<const llvm::Value*, Value*> identifiers;
...
static string value_name(const llvm::Value &v) {
...
static Type* llvm_type2alive(const llvm::Type *ty, ostream *out) {
  switch (ty->getTypeID()) {
  ...
  case llvm::Type::StructTyID: {
    ...
    if (!cache) {
      ...
      for (auto e : cast<llvm::StructType>(ty)->elements()) {
        if (auto ty = llvm_type2alive(e, out))
          elems.push_back(ty);
        else
          return nullptr;
      }
...
static Value* get_operand(llvm::Value *v, ostream *out) {
  if (isa<llvm::Instruction>(v) || isa<llvm::Argument>(v))
    return identifiers[v];

  auto ty = llvm_type2alive(v->getType(), out);
  if (!ty)
    return nullptr;
...
static BasicBlock& getBB(const llvm::BasicBlock *bb) {
  return current_fn->getBB(value_name(*bb));
}

#define PARSE_UNOP()                            \
  auto ty = llvm_type2alive(i.getType(), out);  \
  auto val = get_operand(i.getOperand(0), out); \
  if (!ty || !val)                              \
    return error(i)

#define PARSE_BINOP()                           \
  auto ty = llvm_type2alive(i.getType(), out);  \
  auto a = get_operand(i.getOperand(0), out);   \
  auto b = get_operand(i.getOperand(1), out);   \
  if (!ty || !a || !b)                          \
    return error(i)
...
#define PARSE_TRIOP()                           \
  ...
  auto a = get_operand(i.getOperand(0), out);   \
  auto b = get_operand(i.getOperand(1), out);   \
  auto c = get_operand(i.getOperand(2), out);   \
  if (!ty || !a || !b || !c)                    \
    return error(i)
...
RetTy llvm2alive::visitBinaryOperator(llvm::BinaryOperator &i) {
  PARSE_BINOP();
  BinOp::Op alive_op;
  switch (i.getOpcode()) {
...
RetTy llvm2alive::visitSExtInst(llvm::SExtInst &i) {
  PARSE_UNOP();
  RETURN_IDENTIFIER(make_unique<ConversionOp>(*ty, value_name(i), *val,
                                              ConversionOp::SExt));
}

RetTy llvm2alive::visitZExtInst(llvm::ZExtInst &i) {
  PARSE_UNOP();
  RETURN_IDENTIFIER(make_unique<ConversionOp>(*ty, value_name(i), *val,
                                              ConversionOp::ZExt));
}

RetTy llvm2alive::visitTruncInst(llvm::TruncInst &i) {
  PARSE_UNOP();
  RETURN_IDENTIFIER(make_unique<ConversionOp>(*ty, value_name(i), *val,
                                              ConversionOp::Trunc));
}

RetTy llvm2alive::visitCallInst(llvm::CallInst &i) {
...
RetTy llvm2alive::visitICmpInst(llvm::ICmpInst &i) {
  PARSE_BINOP();
  ICmp::Cond cond;
...
RetTy llvm2alive::visitSelectInst(llvm::SelectInst &i) {
  PARSE_TRIOP();
  RETURN_IDENTIFIER(make_unique<Select>(*ty, value_name(i), *a, *b, *c));
}

RetTy llvm2alive::visitExtractElementInst(llvm::ExtractElementInst &i) {
  PARSE_BINOP();
  RETURN_IDENTIFIER(make_unique<BinOp>(*ty, value_name(i), *a, *b,
                                       BinOp::ExtractValue));
}

RetTy llvm2alive::visitPHINode(llvm::PHINode &i) {
  auto ty = llvm_type2alive(i.getType(), out);
  if (!ty)
    return error(i);
  ...
  for (unsigned idx = 0, e = i.getNumIncomingValues(); idx != e; ++idx) {
    auto op = get_operand(i.getIncomingValue(idx), out);
    if (!op)
      return error(i);
...
RetTy llvm2alive::visitSwitchInst(llvm::SwitchInst &i) {
  ...
  for (auto &Case : i.cases()) {
    op->addTarget(*get_operand(Case.getCaseValue(), out),
                  getBB(Case.getCaseSuccessor()));
  }
  return op;
}

RetTy llvm2alive::visitReturnInst(llvm::ReturnInst &i) {
...
RetTy llvm2alive::visitUnreachableInst(llvm::UnreachableInst &i) {
  auto fals = get_operand(llvm::ConstantInt::getFalse(i.getContext()), out);
  return make_unique<Assume>(*fals, /*if_non_poison=*/false);
}

RetTy llvm2alive::visitIntrinsicInst(llvm::IntrinsicInst &i) {
...
RetTy llvm2alive::error(llvm::Instruction &i) {
  *out << "Unsupported instruction: " << i << '\n';
  return {};
}

bool llvm2alive::handleMetadata(llvm::Instruction &llvm_i, Instr &i, BasicBlock &BB) {
...
optional<Function> llvm2alive::run() {
  reset_state();

  auto type = llvm_type2alive(f.getReturnType(), out);
  if (!type)
    return {};
  ...
  for (auto &arg : f.args()) {
    auto ty = llvm_type2alive(arg.getType(), out);
    if (!ty)
      return {};
