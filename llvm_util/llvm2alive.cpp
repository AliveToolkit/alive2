...
FCmp::Cond parse_fcmp_cond(llvm::FCmpInst::Predicate pred) {
  FCmp::Cond cond;
  switch (pred) {
  case llvm::CmpInst::FCMP_OEQ:cond = FCmp::OEQ;
    break;
  case llvm::CmpInst::FCMP_OGT:cond = FCmp::OGT;
    break;
  case llvm::CmpInst::FCMP_OGE:cond = FCmp::OGE;
    break;
  case llvm::CmpInst::FCMP_OLT:cond = FCmp::OLT;
    break;
  case llvm::CmpInst::FCMP_OLE:cond = FCmp::OLE;
    break;
  case llvm::CmpInst::FCMP_ONE:cond = FCmp::ONE;
    break;
  case llvm::CmpInst::FCMP_ORD:cond = FCmp::ORD;
    break;
  case llvm::CmpInst::FCMP_UEQ:cond = FCmp::UEQ;
    break;
  case llvm::CmpInst::FCMP_UGT:cond = FCmp::UGT;
    break;
  case llvm::CmpInst::FCMP_UGE:cond = FCmp::UGE;
    break;
  case llvm::CmpInst::FCMP_ULT:cond = FCmp::ULT;
    break;
  case llvm::CmpInst::FCMP_ULE:cond = FCmp::ULE;
    break;
  case llvm::CmpInst::FCMP_UNE:cond = FCmp::UNE;
    break;
  case llvm::CmpInst::FCMP_UNO:cond = FCmp::UNO;
    break;
  case llvm::CmpInst::FCMP_TRUE:cond = FCmp::TRUE;
    break;
  case llvm::CmpInst::FCMP_FALSE:cond = FCmp::FALSE;
    break;
  default:UNREACHABLE();
  }
  return cond;
...
map<const llvm::AllocaInst *, std::pair < Alloc * , bool>>
allocs;
...
template<typename T> uint64_t alignment(T &i, llvm::Type *ty) const {
  auto a = i.getAlign().value();
  return a != 0 ? a : DL().getABITypeAlign(ty).value();
}

template<typename T> uint64_t pref_alignment(T &i, llvm::Type *ty) const {
  auto a = i.getAlign().value();
...
auto get_operand(llvm::Value *v) {
  return llvm_util::get_operand(
      v, [this](auto I) { return convert_constexpr(I); },
...
RetTy visitUnaryOperator(llvm::UnaryOperator &i) {
  ...
  FpUnaryOp::Op op;
  switch (i.getOpcode()) {
  case llvm::Instruction::FNeg:op = FpUnaryOp::FNeg;
    break;
  default:return error(i);
  }
...
RetTy visitBinaryOperator(llvm::BinaryOperator &i) {
  ...
  bool is_fp = false;
  switch (i.getOpcode()) {
  case llvm::Instruction::Add:alive_op = BinOp::Add;
    break;
  case llvm::Instruction::Sub:alive_op = BinOp::Sub;
    break;
  case llvm::Instruction::Mul:alive_op = BinOp::Mul;
    break;
  case llvm::Instruction::SDiv:alive_op = BinOp::SDiv;
    break;
  case llvm::Instruction::UDiv:alive_op = BinOp::UDiv;
    break;
  case llvm::Instruction::SRem:alive_op = BinOp::SRem;
    break;
  case llvm::Instruction::URem:alive_op = BinOp::URem;
    break;
  case llvm::Instruction::Shl:alive_op = BinOp::Shl;
    break;
  case llvm::Instruction::AShr:alive_op = BinOp::AShr;
    break;
  case llvm::Instruction::LShr:alive_op = BinOp::LShr;
    break;
  case llvm::Instruction::And:alive_op = BinOp::And;
    break;
  case llvm::Instruction::Or:alive_op = BinOp::Or;
    break;
  case llvm::Instruction::Xor:alive_op = BinOp::Xor;
    break;
  case llvm::Instruction::FAdd:fp_op = FpBinOp::FAdd;
    is_fp = true;
    break;
  case llvm::Instruction::FSub:fp_op = FpBinOp::FSub;
    is_fp = true;
    break;
  case llvm::Instruction::FMul:fp_op = FpBinOp::FMul;
    is_fp = true;
    break;
  case llvm::Instruction::FDiv:fp_op = FpBinOp::FDiv;
    is_fp = true;
    break;
  case llvm::Instruction::FRem:fp_op = FpBinOp::FRem;
    is_fp = true;
    break;
  default:return error(i);
  }
  ...
  if (is_fp)
    return make_unique<FpBinOp>(*ty, value_name(i), *a, *b, fp_op,
                                parse_fmath(i));
...
RetTy visitCastInst(llvm::CastInst &i) {
  PARSE_UNOP();
  {
    ...
    switch (i.getOpcode()) {
    case llvm::Instruction::SExt:op = ConversionOp::SExt;
      break;
    case llvm::Instruction::ZExt:op = ConversionOp::ZExt;
      break;
    case llvm::Instruction::Trunc:op = ConversionOp::Trunc;
      break;
    case llvm::Instruction::BitCast:op = ConversionOp::BitCast;
      break;
    case llvm::Instruction::PtrToInt:op = ConversionOp::Ptr2Int;
      break;
    case llvm::Instruction::IntToPtr:op = ConversionOp::Int2Ptr;
      break;
    default:has_non_fp = false;
      break;
    }
    if (has_non_fp) {
      unsigned flags = 0;
  ...
  switch (i.getOpcode()) {
  case llvm::Instruction::SIToFP:op = FpConversionOp::SIntToFP;
    break;
  case llvm::Instruction::UIToFP:op = FpConversionOp::UIntToFP;
    break;
  case llvm::Instruction::FPToSI:op = FpConversionOp::FPToSInt;
    break;
  case llvm::Instruction::FPToUI:op = FpConversionOp::FPToUInt;
    break;
  case llvm::Instruction::FPExt:op = FpConversionOp::FPExt;
    break;
  case llvm::Instruction::FPTrunc:op = FpConversionOp::FPTrunc;
    break;
  default:return error(i);
  }
...
RetTy visitCallInst(llvm::CallInst &i, bool approx = false) {
  ...
  if (auto *iasm = dyn_cast<llvm::InlineAsm>(i.getCalledOperand())) {
    ...
    call =
        make_unique<InlineAsm>(*ty, value_name(i), iasm->getAsmString(),
  ...
  for (uint64_t argidx = 0, nargs = i.arg_size(); argidx < nargs; ++argidx) {
    ...
    ParamAttrs pattr =
        argidx < param_attrs.size() ? param_attrs[argidx] : ParamAttrs();
    ...
    if (i.paramHasAttr(argidx, llvm::Attribute::NoUndef)) {
      if (i.getArgOperand(argidx)->getType()->isAggregateType())
        ...
        // padding
        return errorAttr(
            i.getAttributeAtIndex(argidx, llvm::Attribute::NoUndef));
    }
...
RetTy visitMemTransferInst(llvm::MemTransferInst &i) {
  ...
  return make_unique<Memcpy>(
      *dst, *src, *bytes, i.getDestAlign().valueOrOne().value(),
      i.getSourceAlign().valueOrOne().value(), isa<llvm::MemMoveInst>(&i));
}
...
RetTy visitICmpInst(llvm::ICmpInst &i) {
  ...
  case llvm::CmpInst::ICMP_EQ:cond = ICmp::EQ;
    break;
  case llvm::CmpInst::ICMP_NE:cond = ICmp::NE;
    break;
  case llvm::CmpInst::ICMP_UGT:cond = ICmp::UGT;
    break;
  case llvm::CmpInst::ICMP_UGE:cond = ICmp::UGE;
    break;
  case llvm::CmpInst::ICMP_ULT:cond = ICmp::ULT;
    break;
  case llvm::CmpInst::ICMP_ULE:cond = ICmp::ULE;
    break;
  case llvm::CmpInst::ICMP_SGT:cond = ICmp::SGT;
    break;
  case llvm::CmpInst::ICMP_SGE:cond = ICmp::SGE;
    break;
  case llvm::CmpInst::ICMP_SLT:cond = ICmp::SLT;
    break;
  case llvm::CmpInst::ICMP_SLE:cond = ICmp::SLE;
    break;
  default:UNREACHABLE();
  }
...
RetTy visitGetElementPtrInst(llvm::GetElementPtrInst &i) {
  ...
  for (auto I = llvm::gep_type_begin(i), E = llvm::gep_type_end(i); I != E;
       ++I) {
      ...
      if (auto opvty = dyn_cast<llvm::VectorType>(opty)) {
        ...
        for (unsigned i = 0; i < opvty->getElementCount().getKnownMinValue();
          ...
          offsets.push_back(llvm::ConstantInt::get(
              ofs_ty, (uint64_t) gep_struct_ofs(structTy, constofs)));
        }

        auto ofs_vector =
            llvm::ConstantVector::get({offsets.data(), offsets.size()});
        gep->addIdx(1, *get_operand(ofs_vector));
      } else {
        gep->addIdx(
            1, *make_intconst(gep_struct_ofs(structTy, I.getOperand()), 64));
      }
      continue;
    }
...
LifetimeKind getLifetimeKind(llvm::IntrinsicInst &i) {
  ...
  if (llvm::all_of(Objs, [](const llvm::Value *V) {
    ...
    return llvm::isa<llvm::Argument, llvm::GlobalVariable,
                     llvm::CallInst>(V);
  }))
  ...
  if (!llvm::all_of(Objs, [](const llvm::Value *V) {
    ...
    if (Objs.size() == 1) {
      ...
      if (GEPI && llvm::isa<llvm::AllocaInst>(
        ...
        if (GEPI->accumulateConstantOffset(DL(), Offset) && !Offset.isZero())
          return LIFETIME_FILLPOISON;
      }
    }
...
RetTy visitIntrinsicInst(llvm::IntrinsicInst &i) {
    ...
    for (unsigned j = 0; j < n; ++j) {
      ...
      if (name == "noundef") {
        ...
        if (bundle.Inputs.size() >= 3) {
          ...
          auto gep = make_unique<GEP>(aptr->getType(),
                                      "#align_adjustedptr" +
                                          to_string(alignopbundle_idx++),
                                      *aptr, false);
  ...
  case llvm::Intrinsic::abs: {
    ...
    BinOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::sadd_with_overflow:op = BinOp::SAdd_Overflow;
      break;
    case llvm::Intrinsic::uadd_with_overflow:op = BinOp::UAdd_Overflow;
      break;
    case llvm::Intrinsic::ssub_with_overflow:op = BinOp::SSub_Overflow;
      break;
    case llvm::Intrinsic::usub_with_overflow:op = BinOp::USub_Overflow;
      break;
    case llvm::Intrinsic::smul_with_overflow:op = BinOp::SMul_Overflow;
      break;
    case llvm::Intrinsic::umul_with_overflow:op = BinOp::UMul_Overflow;
      break;
    case llvm::Intrinsic::sadd_sat:op = BinOp::SAdd_Sat;
      break;
    case llvm::Intrinsic::uadd_sat:op = BinOp::UAdd_Sat;
      break;
    case llvm::Intrinsic::ssub_sat:op = BinOp::SSub_Sat;
      break;
    case llvm::Intrinsic::usub_sat:op = BinOp::USub_Sat;
      break;
    case llvm::Intrinsic::sshl_sat:op = BinOp::SShl_Sat;
      break;
    case llvm::Intrinsic::ushl_sat:op = BinOp::UShl_Sat;
      break;
    case llvm::Intrinsic::cttz:op = BinOp::Cttz;
      break;
    case llvm::Intrinsic::ctlz:op = BinOp::Ctlz;
      break;
    case llvm::Intrinsic::umin:op = BinOp::UMin;
      break;
    case llvm::Intrinsic::umax:op = BinOp::UMax;
      break;
    case llvm::Intrinsic::smin:op = BinOp::SMin;
      break;
    case llvm::Intrinsic::smax:op = BinOp::SMax;
      break;
    case llvm::Intrinsic::abs:op = BinOp::Abs;
      break;
    default:UNREACHABLE();
    }
    FnAttrs attrs;
  ...
  case llvm::Intrinsic::is_constant: {
    ...
    UnaryOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::bitreverse:op = UnaryOp::BitReverse;
      break;
    case llvm::Intrinsic::bswap:op = UnaryOp::BSwap;
      break;
    case llvm::Intrinsic::ctpop:op = UnaryOp::Ctpop;
      break;
    case llvm::Intrinsic::arithmetic_fence:
    case llvm::Intrinsic::expect:
    case llvm::Intrinsic::expect_with_probability:op = UnaryOp::Copy;
      break;
    case llvm::Intrinsic::is_constant:op = UnaryOp::IsConstant;
      break;
    default:UNREACHABLE();
    }
  ...
  case llvm::Intrinsic::vector_reduce_umin: {
    ...
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::vector_reduce_add:op = UnaryReductionOp::Add;
      break;
    case llvm::Intrinsic::vector_reduce_mul:op = UnaryReductionOp::Mul;
      break;
    case llvm::Intrinsic::vector_reduce_and:op = UnaryReductionOp::And;
      break;
    case llvm::Intrinsic::vector_reduce_or:op = UnaryReductionOp::Or;
      break;
    case llvm::Intrinsic::vector_reduce_xor:op = UnaryReductionOp::Xor;
      break;
    case llvm::Intrinsic::vector_reduce_smax:op = UnaryReductionOp::SMax;
      break;
    case llvm::Intrinsic::vector_reduce_smin:op = UnaryReductionOp::SMin;
      break;
    case llvm::Intrinsic::vector_reduce_umax:op = UnaryReductionOp::UMax;
      break;
    case llvm::Intrinsic::vector_reduce_umin:op = UnaryReductionOp::UMin;
      break;
    default:UNREACHABLE();
    }
  ...
  case llvm::Intrinsic::umul_fix_sat: {
    ...
    TernaryOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::fshl:op = TernaryOp::FShl;
      break;
    case llvm::Intrinsic::fshr:op = TernaryOp::FShr;
      break;
    case llvm::Intrinsic::smul_fix:op = TernaryOp::SMulFix;
      break;
    case llvm::Intrinsic::umul_fix:op = TernaryOp::UMulFix;
      break;
    case llvm::Intrinsic::smul_fix_sat:op = TernaryOp::SMulFixSat;
      break;
    case llvm::Intrinsic::umul_fix_sat:op = TernaryOp::UMulFixSat;
      break;
    default:UNREACHABLE();
    }
  ...
  case llvm::Intrinsic::experimental_constrained_fmuladd: {
    ...
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::fma:
    case llvm::Intrinsic::experimental_constrained_fma:op = FpTernaryOp::FMA;
      break;
    case llvm::Intrinsic::fmuladd:
    case llvm::Intrinsic::experimental_constrained_fmuladd:op = FpTernaryOp::MulAdd;
      break;
    default:UNREACHABLE();
    }
  ...
  case llvm::Intrinsic::experimental_constrained_maximum: {
    ...
    FpBinOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::copysign:op = FpBinOp::CopySign;
      break;
    case llvm::Intrinsic::minnum:
    case llvm::Intrinsic::experimental_constrained_minnum:op = FpBinOp::FMin;
      break;
    case llvm::Intrinsic::maxnum:
    case llvm::Intrinsic::experimental_constrained_maxnum:op = FpBinOp::FMax;
      break;
    case llvm::Intrinsic::minimum:
    case llvm::Intrinsic::experimental_constrained_minimum:op = FpBinOp::FMinimum;
      break;
    case llvm::Intrinsic::maximum:
    case llvm::Intrinsic::experimental_constrained_maximum:op = FpBinOp::FMaximum;
      break;
    case llvm::Intrinsic::experimental_constrained_fadd:op = FpBinOp::FAdd;
      break;
    case llvm::Intrinsic::experimental_constrained_fsub:op = FpBinOp::FSub;
      break;
    case llvm::Intrinsic::experimental_constrained_fmul:op = FpBinOp::FMul;
      break;
    case llvm::Intrinsic::experimental_constrained_fdiv:op = FpBinOp::FDiv;
      break;
    default:UNREACHABLE();
    }
    return make_unique<FpBinOp>(*ty, value_name(i), *a, *b, op,
                                parse_fmath(i), parse_rounding(i),
                                parse_exceptions(i));
  }
  ...
  case llvm::Intrinsic::experimental_constrained_trunc: {
    ...
    FpUnaryOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::canonicalize:op = FpUnaryOp::Canonicalize;
      break;
    case llvm::Intrinsic::fabs:op = FpUnaryOp::FAbs;
      break;
    case llvm::Intrinsic::ceil:
    case llvm::Intrinsic::experimental_constrained_ceil:op = FpUnaryOp::Ceil;
      break;
    case llvm::Intrinsic::floor:
    case llvm::Intrinsic::experimental_constrained_floor:op = FpUnaryOp::Floor;
      break;
    case llvm::Intrinsic::rint:
    case llvm::Intrinsic::experimental_constrained_rint:op = FpUnaryOp::RInt;
      break;
    case llvm::Intrinsic::nearbyint:
    case llvm::Intrinsic::experimental_constrained_nearbyint:op = FpUnaryOp::NearbyInt;
      break;
    case llvm::Intrinsic::round:
    case llvm::Intrinsic::experimental_constrained_round:op = FpUnaryOp::Round;
      break;
    case llvm::Intrinsic::roundeven:
    case llvm::Intrinsic::experimental_constrained_roundeven:op = FpUnaryOp::RoundEven;
      break;
    case llvm::Intrinsic::sqrt:
    case llvm::Intrinsic::experimental_constrained_sqrt:op = FpUnaryOp::Sqrt;
      break;
    case llvm::Intrinsic::trunc:
    case llvm::Intrinsic::experimental_constrained_trunc:op = FpUnaryOp::Trunc;
      break;
    default:UNREACHABLE();
    }
    return make_unique<FpUnaryOp>(*ty, value_name(i), *val, op,
                                  parse_fmath(i), parse_rounding(i),
                                  parse_exceptions(i));
  }
  ...
  case llvm::Intrinsic::experimental_constrained_llround: {
    ...
    FpConversionOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::experimental_constrained_sitofp:op = FpConversionOp::SIntToFP;
      break;
    case llvm::Intrinsic::experimental_constrained_uitofp:op = FpConversionOp::UIntToFP;
      break;
    case llvm::Intrinsic::experimental_constrained_fptosi:op = FpConversionOp::FPToSInt;
      break;
    case llvm::Intrinsic::experimental_constrained_fptoui:op = FpConversionOp::FPToUInt;
      break;
    case llvm::Intrinsic::experimental_constrained_fpext:op = FpConversionOp::FPExt;
      break;
    case llvm::Intrinsic::fptrunc_round:
    case llvm::Intrinsic::experimental_constrained_fptrunc:op = FpConversionOp::FPTrunc;
      break;
    case llvm::Intrinsic::lrint:
    case llvm::Intrinsic::experimental_constrained_lrint:
    case llvm::Intrinsic::llrint:
    case llvm::Intrinsic::experimental_constrained_llrint:op = FpConversionOp::LRInt;
      break;
    case llvm::Intrinsic::lround:
    case llvm::Intrinsic::experimental_constrained_lround:
    case llvm::Intrinsic::llround:
    case llvm::Intrinsic::experimental_constrained_llround:op = FpConversionOp::LRound;
      break;
    default:UNREACHABLE();
    }
    FnAttrs attrs;
    parse_fn_attrs(i, attrs);
    RETURN_FNATTRS(make_unique<FpConversionOp>(*ty, value_name(i), *val, op,
                                               parse_rounding(i),
                                               parse_exceptions(i)),
                   attrs);
  }
  ...
  case llvm::Intrinsic::experimental_constrained_fcmps: {
    ...
    return make_unique<FCmp>(*ty, value_name(i), cond, *a, *b,
                             FastMathFlags(), parse_exceptions(i),
                             fcmp->isSignaling());
  }
  case llvm::Intrinsic::is_fpclass: {
    ...
    TestOp::Op op;
    switch (i.getIntrinsicID()) {
    case llvm::Intrinsic::is_fpclass:op = TestOp::Is_FPClass;
      break;
    default:UNREACHABLE();
    }
  ...
  case llvm::Intrinsic::lifetime_end: {
    ...
    case LIFETIME_START:return make_unique<StartLifetime>(*b);
    case LIFETIME_START_FILLPOISON:BB->addInstr(make_unique<StartLifetime>(*b));
      return make_unique<FillPoison>(*b);
    case LIFETIME_FREE:return make_unique<EndLifetime>(*b);
    case LIFETIME_FILLPOISON:return make_unique<FillPoison>(*b);
    case LIFETIME_NOP:return NOP(i);
    case LIFETIME_UNKNOWN:return error(i);
    }
  }
  ...
  case llvm::Intrinsic::sideeffect: {
    ...
    attrs.set(FnAttrs::NoThrow);
    return make_unique<FnCall>(Type::voidTy, "", "#sideeffect",
                               std::move(attrs));
  }
  ...
  case llvm::Intrinsic::prefetch:return NOP(i);
...
RetTy visitShuffleVectorInst(llvm::ShuffleVectorInst &i) {
  ...
  for (auto m : i.getShuffleMask())
    mask.push_back(m);
  return make_unique<ShuffleVector>(*ty, value_name(i), *a, *b,
                                    std::move(mask));
}
  ...
  for (auto &[ID, Node] : MDs) {
    ...
    case LLVMContext::MD_range: {
      ...
      for (auto &Op : Node->operands()) {
        args.emplace_back(
            get_operand(llvm::mdconst::extract<llvm::ConstantInt>(Op)));
      }

      AssumeVal::Kind op;
      const char *str = nullptr;
      switch (ID) {
      case LLVMContext::MD_align:op = AssumeVal::Align;
        str = "_align";
        break;
      case LLVMContext::MD_nonnull:op = AssumeVal::NonNull;
        str = "_nonnull";
        break;
      case LLVMContext::MD_range:op = AssumeVal::Range;
        str = "_range";
        break;
      default:UNREACHABLE();
      }

      auto assume = make_unique<AssumeVal>(i->getType(), i->getName() + str,
                                           *i, std::move(args), op);
    ...
    case LLVMContext::MD_noundef:BB->addInstr(make_unique<Assume>(*i, Assume::WellDefined));
      break;
    ...
    case LLVMContext::MD_unpredictable:break;
...
unique_ptr <Instr> handleRangeAttrNoInsert(const llvm::Attribute &attr,
                                           Value &val) {
  auto CR = attr.getValueAsConstantRange();
  vector < Value * > bounds{make_intconst(CR.getLower()),
                            make_intconst(CR.getUpper())};
  return make_unique<AssumeVal>(val.getType(), "%#range_" + val.getName(),
                                val, std::move(bounds), AssumeVal::Range);
}
...
bool handleParamAttrs(const llvm::AttributeSet &aset, ParamAttrs &attrs,
  ...
  for (const llvm::Attribute &llvmattr : aset) {
    ...
    case llvm::Attribute::InReg:attrs.set(ParamAttrs::InReg);
      break;

    case llvm::Attribute::SExt:attrs.set(ParamAttrs::SignExt);
      break;

    case llvm::Attribute::ZExt:attrs.set(ParamAttrs::ZeroExt);
      break;
    ...
    case llvm::Attribute::NonNull:attrs.set(ParamAttrs::NonNull);
      break;

    case llvm::Attribute::NoCapture:attrs.set(ParamAttrs::NoCapture);
      break;

    case llvm::Attribute::ReadOnly:attrs.set(ParamAttrs::NoWrite);
      break;

    case llvm::Attribute::WriteOnly:attrs.set(ParamAttrs::NoRead);
      break;
    ...
    case llvm::Attribute::Dereferenceable:attrs.set(ParamAttrs::Dereferenceable);
      attrs.derefBytes =
          max(attrs.derefBytes, llvmattr.getDereferenceableBytes());
      break;

    case llvm::Attribute::DereferenceableOrNull:attrs.set(ParamAttrs::DereferenceableOrNull);
      attrs.derefOrNullBytes = max(attrs.derefOrNullBytes,
                                   llvmattr.getDereferenceableOrNullBytes());
      break;

    case llvm::Attribute::Alignment:attrs.set(ParamAttrs::Align);
      attrs.align = max(attrs.align, llvmattr.getAlignment()->value());
      break;

    case llvm::Attribute::NoUndef:attrs.set(ParamAttrs::NoUndef);
      break;

    case llvm::Attribute::Range:newval = handleRangeAttr(llvmattr, val);
      break;

    case llvm::Attribute::NoFPClass:attrs.set(ParamAttrs::NoFPClass);
      attrs.nofpclass = (uint16_t) llvmattr.getNoFPClass();
      break;

    case llvm::Attribute::Returned:attrs.set(ParamAttrs::Returned);
      break;

    case llvm::Attribute::AllocatedPointer:attrs.set(ParamAttrs::AllocPtr);
      break;

    case llvm::Attribute::AllocAlign:attrs.set(ParamAttrs::AllocAlign);
      break;
...
void handleRetAttrs(const llvm::AttributeSet &aset, FnAttrs &attrs) {
  for (const llvm::Attribute &llvmattr : aset) {
    ...
    switch (llvmattr.getKindAsEnum()) {
    case llvm::Attribute::SExt:attrs.set(FnAttrs::SignExt);
      break;
    case llvm::Attribute::ZExt:attrs.set(FnAttrs::ZeroExt);
      break;
    case llvm::Attribute::NoAlias:attrs.set(FnAttrs::NoAlias);
      break;
    case llvm::Attribute::NonNull:attrs.set(FnAttrs::NonNull);
      break;
    case llvm::Attribute::NoUndef:attrs.set(FnAttrs::NoUndef);
      break;

    case llvm::Attribute::Dereferenceable:attrs.set(FnAttrs::Dereferenceable);
      attrs.derefBytes =
          max(attrs.derefBytes, llvmattr.getDereferenceableBytes());
      break;
    case llvm::Attribute::DereferenceableOrNull:attrs.set(FnAttrs::DereferenceableOrNull);
      attrs.derefOrNullBytes = max(attrs.derefOrNullBytes,
                                   llvmattr.getDereferenceableOrNullBytes());
      break;

    case llvm::Attribute::Alignment:attrs.set(FnAttrs::Align);
      attrs.align = max(attrs.align, llvmattr.getAlignment()->value());
      break;

    case llvm::Attribute::NoFPClass:attrs.set(FnAttrs::NoFPClass);
      attrs.nofpclass = (uint16_t) llvmattr.getNoFPClass();
      break;
...
static FPDenormalAttrs::Type parse_fp_denormal_str(string_view str) {
  ...
  if (str == "preserve-sign")
    return FPDenormalAttrs::PreserveSign;
  if (str == "positive-zero")
...
void handleFnAttrs(const llvm::AttributeSet &aset, FnAttrs &attrs) {
  for (const llvm::Attribute &llvmattr : aset) {
    ...
    switch (llvmattr.getKindAsEnum()) {
    case llvm::Attribute::NoFree:attrs.set(FnAttrs::NoFree);
      break;
    ...
    case llvm::Attribute::NoReturn:attrs.set(FnAttrs::NoReturn);
      break;

    case llvm::Attribute::WillReturn:attrs.set(FnAttrs::WillReturn);
      break;

    case llvm::Attribute::NullPointerIsValid:attrs.set(FnAttrs::NullPointerIsValid);
      break;
...
void parse_fn_attrs(const llvm::CallInst &i, FnAttrs &attrs,
  ...
  llvm::AttributeList attrs_fndef =
      fn ? fn->getAttributes() : llvm::AttributeList();
