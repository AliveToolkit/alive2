...
define void @tgt() {
  %minusone = fdiv float 1.0, -1.0
  %minuszero = fdiv float 0.0, -1.0
  %minusinf = fdiv float 1.0, -0.0
  %minusnan = fdiv float 0.0, -0.0

  call void @f(float %minusone, float %minusone, float %minuszero, float %minuszero, float %minusinf, float %minusinf, float %minusnan, float %minusnan)
  ret void
}
